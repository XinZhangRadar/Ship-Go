...
class SARD(BaseModel):
    def __init__(self, networks, losses, sample_num, task, optimizers, num_env, image_size, ema_scheduler=None, **kwargs):
        ''' must to init BaseModel with kwargs '''
        super(SARD, self).__init__(**kwargs)

        ''' networks, dataloder, optimizers, losses, etc. '''
        self.loss_fn = losses[0]
        self.netG = networks[0]
        self.netG.set_condition(image_size,image_size, num_env)
        ...
        self.optG = torch.optim.Adam(list(filter(lambda p: p.requires_grad, self.netG.parameters())), **optimizers[0])
        #self.optG = torch.optim.Adam(list(lambda p: p.requires_grad, self.netG.parameters()), **optimizers[0])
        ...
        if self.opt['distributed']:
            self.netG.module.set_loss(self.loss_fn)
            self.netG.module.set_new_noise_schedule(phase=self.phase)
            #self.netG.module.set_condition(image_size,image_size, num_env)
    ...
    def set_input(self, data):
        ...
        self.path = data['path']
        self.batch_size = len(data['path'])
        self.image_env = data.get('image_env')
    ...
    def train_step(self):
        ...
        for train_data in tqdm.tqdm(self.phase_loader):
            ...
            # print(train_data['image_env'])
            self.optG.zero_grad()
            loss = self.netG(self.gt_image, self.cond_image, mask=self.mask, image_env = self.image_env) 
            loss.backward()
            self.optG.step()
        ...
        with torch.no_grad():
            for val_data in tqdm.tqdm(self.val_loader):
                ...
                if self.opt['distributed']:
                    if self.task in ['inpainting','uncropping']:
                        self.output, self.visuals = self.netG.module.restoration(self.cond_image, y_t=self.cond_image, 
                            y_0=self.gt_image, mask=self.mask, sample_num=self.sample_num, image_env = self.image_env)
                    else:
                        self.output, self.visuals = self.netG.module.restoration(self.cond_image, sample_num=self.sample_num)
                else:
                    if self.task in ['inpainting','uncropping']:
    ...
    def test(self):
        ...
        with torch.no_grad():
            for phase_data in tqdm.tqdm(self.phase_loader):
                self.set_input(phase_data)
                #import pdb;pdb.set_trace()
                #self.image_env = torch.zeros_like(self.image_env) # offshore
                #self.image_env = torch.ones_like(self.image_env) # inshore
                if self.opt['distributed']:
                    if self.task in ['inpainting','uncropping']:
                        self.output, self.visuals = self.netG.module.restoration(self.cond_image, y_t=self.cond_image, 
                            y_0=self.gt_image, mask=self.mask, sample_num=self.sample_num,image_env = self.image_env)
